...
<div class="container mt-7">
  <div v-if="currentDate">
    <h1 class="m-0">{{ currentDate.format('MMMM, YYYY') }}</h1>
    <h3 class="mb-4">{{ currentDate.format('D MMM') }} - {{ nextDate.subtract(1, 'day').format('D MMM') }}</h3>

    <div class="d-flex">
      <div class="d-flex grouped-buttons">
        <RouterLink class="button" :to="{ name: 'MonthView', params: { year: previousDate.year(), month: previousDate.month() + 1 } }">
        ...
        <RouterLink class="button" :to="{ name: 'MonthView', params: { year: nextDate.year(), month: nextDate.month() + 1 } }">
          <span class="mr-1">Next month</span>
      ...
      <RouterLink class="button button-outline ml-1" v-if="showTodayButton()" :to="{ name: 'MonthView', params: {} }">
  ...
  <div v-if="currentDate">
      ...
      <table>
        ...
        <tfoot>
          <tr>
            ...
            <td></td>
            <td></td>
            <td class="text-right">Total in month:</td>
    ...
    <div v-else>
      <div class="panel p-4 d-flex align-center">
        ...
        <span class="ml-2">Empty month!</span>
      </div>
    </div>
...
async function loadView() {
  ...
  if (route.params.year && route.params.month) {
    ...
    currentDate.value = dayjs().utc().startOf('month');
  }
  currentDate.value = currentDate.value.utc().hour(0).minute(0).second(0);

  nextDate.value = currentDate.value.utc().add(1, 'month');
  previousDate.value = currentDate.value.utc().subtract(1, 'month');
...
function showTodayButton() {
  ...
  if (currentDate.value.month() === dayjs().month() && currentDate.value.year() === dayjs().year()) {
    return false;
  }
...
onMounted(() => {
  document.title = `Month - ${process.env.VUE_APP_TITLE_POST}`;
  loadView();
});
